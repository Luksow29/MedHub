...
const AppointmentsPage: React.FC<AppointmentsPageProps> = ({ user, onLogout }) => {
  const [isPatientModalOpen, setIsPatientModalOpen] = useState(false);
  const [isAppointmentModalOpen, setIsAppointmentModalOpen] = useState(false);
  const [activeTab, setActiveTab] = useState<'list' | 'calendar' | 'scheduler' | 'waitlist'>('list');
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-3xl font-semibold text-slate-800">
          {getBilingualLabel("Appointment Management", "சந்திப்பு நிர்வாகம்")}
        </h2>
