...
const SchedulerPage: React.FC<SchedulerPageProps> = ({ user, onLogout }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      console.error('Scheduler data fetch error:', err.message);
  ...
  const handleAppointmentCreated = () => {
    // Refresh data after appointment creation
    fetchData();
  };
  ...
  return (
    <MainLayout
      ...
      breadcrumbs={[
        ...
        { label: getBilingualLabel('Advanced Scheduler', 'மேம்பட்ட அட்டவணையாளர்') }
      ]}
      isLoading={isLoading}
    >
      {/* Error Display */}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mb-6 gap-4">
        <h2 className="text-3xl font-semibold text-slate-800">
          {getBilingualLabel("Advanced Scheduler", "மேம்பட்ட அட்டவணையாளர்")}
        </h2>
