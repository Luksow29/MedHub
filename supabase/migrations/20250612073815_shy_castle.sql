...
BEGIN
  ...
  FOR slot_record IN
    ...
    FROM time_slots ts
    WHERE ts.user_id = p_user_id_param
      AND ts.day_of_week = day_of_week_num
      AND ts.is_available = true
  LOOP
    -- Convert times to timestamps for the specific date
    current_time := p_appointment_date + slot_record.start_time;
    end_time := p_appointment_date + slot_record.end_time - (p_duration_minutes || ' minutes')::interval;
    slot_duration := (p_duration_minutes + slot_record.buffer_time || ' minutes')::interval;
    
    -- Generate time slots within this availability window
    WHILE current_time <= end_time LOOP
      -- Check if this time slot conflicts with existing appointments
      IF NOT EXISTS (
        SELECT 1 FROM appointments ea
        WHERE ea.user_id = p_user_id_param
          AND ea.date = p_appointment_date
          AND ea.status NOT IN ('cancelled', 'completed')
          AND (
            (ea.time <= current_time::time AND (ea.time + (ea.duration || ' minutes')::interval)::time > current_time::time)
            OR
            (current_time::time <= ea.time AND (current_time::time + (p_duration_minutes || ' minutes')::interval)::time > ea.time)
          )
      ) THEN
        -- Return this available slot
        suggested_time := current_time::time;
