...
BEGIN
  ...
  WHERE 
    ...
    AND (
      ...
      (a.time <= appointment_time AND (a.time + (a.duration || ' minutes')::interval)::time > appointment_time)
      OR
      (appointment_time <= a.time AND (appointment_time + (duration_minutes || ' minutes')::interval)::time > a.time)
    );
END;
$$ LANGUAGE plpgsql;
...
BEGIN
  ...
  SELECT 
    generate_series(
      ts.start_time,
      ts.end_time - (duration_minutes || ' minutes')::interval,
      (duration_minutes + ts.buffer_time || ' minutes')::interval
    )::time as suggested_time,
    duration_minutes as available_duration
  FROM time_slots_for_day ts
  WHERE NOT EXISTS (
    SELECT 1 FROM existing_appointments ea
    WHERE (ea.time <= generate_series(
      ts.start_time,
      ts.end_time - (duration_minutes || ' minutes')::interval,
      (duration_minutes + ts.buffer_time || ' minutes')::interval
    )::time 
    AND (ea.time + (ea.duration || ' minutes')::interval)::time > generate_series(
      ts.start_time,
      ts.end_time - (duration_minutes || ' minutes')::interval,
      (duration_minutes + ts.buffer_time || ' minutes')::interval
    )::time)
  )
  ORDER BY suggested_time;
END;
$$ LANGUAGE plpgsql;
