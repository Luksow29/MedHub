...
const DashboardPageEnhanced: React.FC<DashboardPageEnhancedProps> = ({ user, onLogout }) => {
  ...
  const [activeTab, setActiveTab] = useState<'appointments' | 'patients' | 'search'>('appointments');
  ...
  const getPageTitle = () => {
    switch (activeTab) {
      case 'appointments':
        return getBilingualLabel('Appointments Dashboard', 'சந்திப்புகள் டாஷ்போர்டு');
      case 'search':
        return getBilingualLabel('Patient Search', 'நோயாளர் தேடல்');
      case 'patients':
        return getBilingualLabel('All Patients', 'அனைத்து நோயாளிகள்');
      default:
        return getBilingualLabel('Dashboard', 'டாஷ்போர்டு');
    }
  };

  return (
    <PrintablePageWrapper pageTitle={getPageTitle()}>
      <div className="min-h-screen bg-slate-100">
        <Header user={user} pageTitle={getPageTitle()} />
        <main className="container mx-auto p-4 md:p-6">
          {error && (
            <div className="bg-red-100 border-l-4 border-red-500 text-red-700 p-4 mb-6 rounded-md shadow print:border print:border-red-500" role="alert">
          ...
          <div className="mb-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
              <div className="bg-white p-6 rounded-lg shadow-md stat-card">
          ...
          {!isLoading && activeTab === 'patients' && (
            <div className="bg-white shadow-lg rounded-lg overflow-hidden page-break-inside-avoid">
                ...
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th scope="col" className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider print:hidden">செயல்கள்</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-slate-200">
                    {patients.map((patient) => (
                      <tr key={patient.id} className="hover:bg-slate-50 transition-colors">
                        ...
                        <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-600 print:hidden">
                          <a
