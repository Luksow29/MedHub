...
const DetailedSummaryModal: React.FC<DetailedSummaryModalProps> = ({ isOpen, onClose, summaryData }) => {
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} title={pageTitle} size="3xl">
        {/* Main container with flex layout to separate content and footer */}
        <div className="flex flex-col" style={{maxHeight: '80vh'}}>

            {/* Scrollable Content Area */}
            <div className="flex-grow overflow-y-auto p-6 printable-content">
                <div id="detailed-summary-printable-content">
                        ...
                        {vitalSigns && (
                            <div className="mb-6">
                                ...
                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                                    {vitalSigns.temperature !== null && <p><span className="font-semibold">Temp:</span> {vitalSigns.temperature}°C</p>}
                                    {vitalSigns.heartRate !== null && <p><span className="font-semibold">HR:</span> {vitalSigns.heartRate} bpm</p>}
                                    {vitalSigns.bloodPressureSystolic !== null && <p><span className="font-semibold">BP:</span> {vitalSigns.bloodPressureSystolic}/{vitalSigns.bloodPressureDiastolic} mmHg</p>}
                                    {vitalSigns.respiratoryRate !== null && <p><span className="font-semibold">RR:</span> {vitalSigns.respiratoryRate}</p>}
                                    {vitalSigns.oxygenSaturation !== null && <p><span className="font-semibold">SpO2:</span> {vitalSigns.oxygenSaturation}%</p>}
                                    {vitalSigns.bmi !== null && <p><span className="font-semibold">BMI:</span> {vitalSigns.bmi}</p>}
